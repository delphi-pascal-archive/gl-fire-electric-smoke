...
// 2.32 - Changed to function (returns int)
function NewtonWorldConvexCast( const newtonWorld : PNewtonWorld; const matrix : PFloat; const target : PFloat; const shape : PNewtonCollision;
                              hitParam : PFloat; userData: Pointer; prefilter : NewtonWorldRayPrefilterCallback;
                              info : PNewtonWorldConvexCastReturnInfo; maxContactsCount : int; threadIndex : int) : Int; cdecl; external{$IFDEF __GPC__}name 'NewtonWorldConvexCast'{$ELSE}NewtonDLL{$ENDIF __GPC__};
...
function NewtonCreateCompoundCollisionFromMesh( const newtonWorld : PNewtonWorld; const mesh : PNewtonMesh; maxSubShapesCount : int; shapeID : Int; subShapeID : int ) : PNewtonCollision; cdecl; external{$IFDEF __GPC__}name 'NewtonCreateCompoundCollisionFromMesh'{$ELSE}NewtonDLL{$ENDIF __GPC__};
...
//
// **********************************************************************************************
function NewtonCreateCompoundBreakable( const NewtonWorld : PNewtonWorld; meshCount : int; const SolidsArray : PNewtonMesh; const ShapeIDArray : PInt; Densities : PFloat; interialFaceMaterial : PInt; ShapeID : Int; debriID : Int; DebriSeparationGap : Float ) : PNewtonCollision; cdecl; external{$IFDEF __GPC__}name 'NewtonCreateCompoundBreakable'{$ELSE}NewtonDLL{$ENDIF __GPC__};
...
// 2.23 - NewtonMeshCreateFromMesh added - SW
...
// 2.26 - Function added
function NewtonMeshConvexHull (const NewtonWorld : PNewtonWorld; count : int; const vertexCloud : PFloat; strideInBytes : int; tolerance : float) : PNewtonMesh; cdecl; external{$IFDEF __GPC__}name 'NewtonMeshConvexHull'{$ELSE}NewtonDLL{$ENDIF __GPC__};
...
// 2.23 - NewtonMeshConvexApproximation added - SW
// 2.26 - Function removed - SW
//function NewtonMeshConvexApproximation (const Mesh : PNewtonMesh; const Word : PNewtonWorld;  maxConvexity : Float; MaxCount : Int; ConvexArray : PNewtonCollision) : Int; cdecl; external{$IFDEF __GPC__}name 'NewtonMeshConvexApproximation'{$ELSE}NewtonDLL{$ENDIF __GPC__};
...
// 2.23 - NewtonMeshBuildFromVertexListIndexList added - SW
...
procedure NewtonMeshGetVertexStreams(const Mesh : PNewtonMesh; vertexStrideInByte : Int; Vertex : PFloat; normalStrideInByte : int; Normal : PFloat; uvStrideInByte0 : Int; uv0 : PFloat; uvStrideInByte1 : Int; uv1 : PFloat);  cdecl; external{$IFDEF __GPC__}name 'NewtonMeshGetVertexStreams'{$ELSE}NewtonDLL{$ENDIF __GPC__};

procedure NewtonMeshGetIndirectVertexStreams(const mesh : PNewtonMesh; vertexStrideInByte : int; vertex : PFloat; vertexIndices : PInt; vertexCount : PInt; normalStrideInByte : int; normal : PFloat; normalIndices : PInt; normalCount : PInt; uvStrideInByte0 : int; uv0 : PFloat; uvIndices0 : PInt; uvCount0 : PInt; uvStrideInByte1 : int; uv1 : PFloat; uvIndices1 : PInt; uvCount1 : PInt); cdecl; external{$IFDEF __GPC__}name 'NewtonMeshGetIndirectVertexStreams'{$ELSE}NewtonDLL{$ENDIF __GPC__};
...
// 2.23 - All functions below added - SW
